...
alter table paradas_faena.paradas      add column version integer;
alter table paradas_faena.input_status add column version integer;
update paradas_faena.paradas      set version = 1 where version is null;
update paradas_faena.input_status set version = 1 where version is null;
alter table paradas_faena.paradas      alter column version set not null;
alter table paradas_faena.input_status alter column version set not null;

-- The foreign keys have to come off before the primary key they point at can move.
alter table paradas_faena.paradas      drop constraint counters_name_fk;
alter table paradas_faena.input_status drop constraint input_status_counters_name_fk;

alter table paradas_faena.counters_name drop constraint counters_name_pk;
alter table paradas_faena.counters_name
    add constraint counters_name_pk primary key (ip, tag, version);

alter table paradas_faena.paradas
    add constraint counters_name_fk foreign key (ip, tag, version)
        references paradas_faena.counters_name (ip, tag, version);
alter table paradas_faena.input_status
...
drop index if exists paradas_faena.counters_name_input_tag_uq;
create unique index counters_name_input_tag_uq
    on paradas_faena.counters_name (ip, input_tag, version)
...
create index idx_paradas_counter on paradas_faena.paradas (ip, tag, version);
create index idx_input_status_counter on paradas_faena.input_status (ip, tag, version);
