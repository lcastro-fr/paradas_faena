...
COUNTERS_8 = [
    CounterConfig(
        ip=IP8, tag="Cont_P1", name="Puesto 1", input_tag="_IO_EM_DI_00", version=1
    ),
    CounterConfig(
        ip=IP8, tag="Cont_P2", name="Puesto 2", input_tag="_IO_EM_DI_01", version=1
    ),
]
COUNTERS_9 = [
    CounterConfig(
        ip=IP9, tag="Cont_P3", name="Puesto 3", input_tag="_IO_P1_DI_00", version=1
    )
]
...
def fast_config(config: Config, **overrides) -> Config:
    ...
    return dataclasses.replace(
        config,
        db=DbConfig("h", 5432, "d", "u", "p", "monitoreo_faena"),
        poll_seconds=0.01,
...
def test_unmapped_counters_are_still_polled_for_stops(stream, config):
    ...
    counters = [
        CounterConfig(ip=IP8, tag="Cont_P1", name="Puesto 1", input_tag=None, version=1)
    ]
    worker = make_worker(PLC_8, counters, FakePlc(IP8), stream, config)
...
def test_a_first_tick_publishes_resync_inputs_and_a_heartbeat(stream, config):
    ...
    assert sorted((e.tag, e.value) for e in events if isinstance(e, InputEdge)) == [
        ("Cont_P1", False),
        ("Cont_P2", True),
    ]
...
def test_the_worker_thread_reconnects_and_then_exits_on_shutdown(stream, config):
    ...
    def factory(ip):
        fake = FakePlc(
            ip, [{"Cont_P1": 5, "Cont_P2": 2, "_IO_EM_DI_00": True, "_IO_EM_DI_01": True}]
        )
